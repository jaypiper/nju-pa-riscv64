...
static Area segments[] = {
  RANGE(&_addr_start, PMEM_END),
  RANGE(FB_ADDR, FB_ADDR + 0x200000),
  RANGE(UART_ADDR, UART_ADDR + 0x8)
};
...
assert(as->ptr);
  ...
  pg_base = (uint64_t*)((pg_base[idx] & PGTABLE_MASK) << 2);
  idx = (_vaddr >> 21) & 0x1ff;
  if(!(pg_base[idx] & VALID_MASK)){
    void* _new_pg = pgalloc_usr(PGSIZE);
    pg_base[idx] = (((uint64_t)_new_pg >> 2) & PGTABLE_MASK) | VALID_MASK;
...
Context *ucontext(AddrSpace *as, Area kstack, void *entry) {
  ...
  // Context* _context = (Context*)as->area.end -1;
